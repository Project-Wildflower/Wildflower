...
/obj/machinery/power/rad_collector/Process()
	...
	if(P)
		if(P.air_contents.gas[GAS_PLASMA] == 0)
			...
			P.air_adjust_gas(GAS_PLASMA, -0.01*drainratio*min(last_rads,max_rads)/max_rads) //fuel cost increases linearly with incoming radiation
...
/obj/machinery/power/rad_collector/proc/receive_pulse(var/pulse_strength)
	if(P && active)
		...
		power_produced = min(100*P.air_contents.gas[GAS_PLASMA]*pulse_strength*pulse_coeff,max_power)
