...
/datum/reagent/toxin/plasma/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	if(alien == IS_NABBER)
		return
	..()
...
/datum/reagent/toxin/plasma/touch_turf(var/turf/simulated/T)
	if(!istype(T))
		return
	T.assume_gas(GAS_PLASMA, volume, T20C)
	remove_self(volume)
...
/datum/reagent/toxin/plasma/oxygen/touch_turf(var/turf/simulated/T)
	if(!istype(T))
		return
	T.assume_gas(GAS_OXYGEN, ceil(volume/2), T20C)
	T.assume_gas(GAS_PLASMA, ceil(volume/2), T20C)
	remove_self(volume)
